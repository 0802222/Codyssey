...
``` text
AICodyssey 사전 평가가 버튼 클릭 시 Readme.md 를 찾을 수 없다고 함
```
...
``` text
1. 브랜치를 따서 찾지 못하는 경우
2. 폴더구조가 여러겹이라 찾지 못하는 경우
```

### 확인

``` text
1. main / master 브랜치만 읽도록 설계되어 있다고 함 => main 브랜치로 변경
2. 최상위 루트 디렉토리만 읽도록 설계되어 있다고 함 => 루트 디렉토리로 이동

위 2문제를 해결했으나 동일증상 발생
3. 파일명이 대문자라 읽지못함 => Readme.md 로 변경
4. AICodyssey 반영시간이 소요될 수 있어, 깃허브에서 자체 수정으로 커밋내역을 남겨보기로 함
