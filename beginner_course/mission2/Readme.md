...
### 3. Ctrl + z (Suspend)
- 상황 : 터미널에서 퀴즈게임이 실행 중일 때  `Ctrl + Z` 를 누르면?
- 현재 파이썬 프로세스가 중지된 상태로 background 로 이동한다. (`SIGTSTP`)
  ![suspend](/docs/screenshots/suspend.png)
- `fg` 명령어를 통해 다시 foreground 로 프로세스를 전환하고, 중단했던 시점부터 게임을 계속 진행할 수 있다. (`SIGCONT`)
  ![fg](/docs/screenshots/fg.png)
- 이는 프로그램의 `try/except` 로 잡을 수 있는 종류의 예외가 아니라, 운영체제 레벨에서 처리되는 시그널 제어 이다.


### 4. 동점 처리 규칙
  - 기존 최고점
  ![기존 최고점](/docs/screenshots/best_score.png)
  - 새로운 최고점 등재
  ![새로운 최고점](/docs/screenshots/best_score_new.png)
- 상황 : 점수가 높은 사람이 명예의 전당에 등재되는데 동점인 경우는?
- 당시에는 `처음 최고점을 받은 사람`이 계속 등재되어 있는 구조였는데, 동료평가를 진행하면서 최근에 최고점을 받은사람으로 갱신하는게 사용자 경험상 버그같아 보이지않고, 재미요소를 줄 수 있다고 생각해서 바꾸기로 했다.
- `최근에 최고점을 받은 사람`이 명예의 전당에 갱신 등재 된다.


### 5. 닉네임 중복 방지
  ![중복 방지](/docs/screenshots/is_duplicate_nickname.png)
- 상황 : 간단한 퀴즈게임에 명예의 전당 등재를 위해 닉네임을 받았는데, 동료평가를 진행하면서 중복된 닉네임 처리가 되어있느냐? 라는 질문을 받았다.

- Java로 프로그래밍할 때는 자연스럽게 고려했던 부분이지만, 이번에는 “간단한 Python 콘솔 프로그램”이라는 생각에 별도의 처리를 하지 않았었다. 이 피드백을 통해, 프로그램의 규모와 상관없이 사용자의 관점에서 예외 상황을 고민하는 시선이 개발자에게 중요하다는 점을 다시 한 번 느꼈다.

- 개선 후에는 `state.json`에 `players` 리스트를 저장하고, 닉네임 입력 시 정규화(공백 제거, 소문자 변환) 후 이미 존재하는지 검사하여, 중복된 닉네임은 거부하고 새로운 닉네임만 추가(`append`)하는 구조로 변경하였다.